		...
		if(!reset_n) begin
			...
			case (chstate)
				...
				chings:begin
					chdone <= 0;
					checkpointing <= 1;
					if ((state==stateIDLE && !memRead && !memWrite) || (state == stateREAD && cacheHit)) begin
						//chstate <= chdones;
						chstate<=chcounts;
					end
				end
			...
			case(state)
				stateIDLE: begin
					...
					if(memRead) begin //If we want a read start a read
						...
						avm_dataMaster_address <= {memAddress[31:2], 2'b0};
						//$display($time,"	tag:%h,	dc_addr: %h\n",memAddress[31:13], memAddress[12:4]);

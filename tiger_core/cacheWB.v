...
/*
   ...
   is parametrised.  2^blockSize is the block size in bytes,
   ...
   On a write if it's a hit, only write to cache. On a miss, check if the cache
   line is dirty in which case write back the dirty line to memory and fetch
   the block we want to write to. Write the word to the fetched block.
...
module CacheWB(input clk,
			 ...
			 input poweroff,
			 input checkpoint,
			 output reg checkpointdone,
			 output reg checkpointing,
			 output reg [9:0] chptdirtycounter,
			 output reg [15:0] rmisscounter,
			 output reg [15:0] wmisscounter,
	...
	parameter stateWRITEBACK = 7;
	parameter stateFETCH_WRITE = 8;
	parameter stateWBDIRTYCACHE = 9;
	parameter stateWBDIRTY = 10;
	parameter stateCLEAN = 11;
	...
	wire [blockSizeBits + tagSizeBits + 1 : 0]cacheData;
	wire [blockSizeBits + tagSizeBits + 1 : 0]cacheQ;
	...
	//modified for WB cache
	reg [blockSizeBits + tagSizeBits + 1 : 0]zcacheQ;
	wire [tagSizeBits - 1 : 0]zcacheTag;
	wire zvalidBit;
	wire [blockSizeBits - 1 : 0]zcacheQData;
	wire zdirtyBit;
	assign zcacheTag = zcacheQ[tagSizeBits : 1];
	assign zvalidBit = zcacheQ[0];
	assign zcacheQData = zcacheQ[blockSizeBits + tagSizeBits : tagSizeBits + 1];
	assign zdirtyBit = zcacheQ[blockSizeBits + tagSizeBits + 1];
	reg [cacheSize - 1 : 0]zflushAddr;
	
	//on write miss, keep the write data and address
	reg [31:0] zmemAddress;
	reg [31:0] zmemWriteData;
	reg zmemRead;
	reg zmemWrite;
	...
	reg flushdone;
	reg chdone;
	reg writebackdirtydone;
	reg pipelineempty;
	
	//modified for checkpoint and rollback
	//reg [15:0] rmisscounter;
	//reg [15:0] wmisscounter;
	reg [9:0] chptdc;
	...
	//modified for WB cache
	reg [blockSize - 3 : 0]writebackWord;
	reg writeclean;
	...
	//modified for WB cache
	assign dirtyBit = cacheQ[blockSizeBits + tagSizeBits + 1];
	...
	assign stall = state == stateFETCH || (state == stateREAD && !cacheHit) 
		...
		//modified for WB cache
		|| (state == stateCLEAN)
		|| (state == stateWRITEBACK) || (state == stateFETCH_WRITE)
		|| (state == stateWBDIRTYCACHE) || (state == stateWBDIRTY);
	...
	assign cacheAddress = fetchDone ? address[cacheSize + blockSize - 1 : blockSize] :
		...
		//modified for checkpoint and rollback
		state == stateCLEAN  ? flushAddr :
		(checkpointing && pipelineempty)  ? flushAddr :
		//(state == stateWBDIRTYCACHE) || ((state == stateWRITEBACK) && checkpointing) /*|| ((state == stateIDLE) && checkpointing && !writebackdirtydone)*/ ? flushAddr :
		((state == stateWRITEBACK) && !checkpointing)||((state == stateREAD) && !cacheHit && dirtyBit) ? zmemAddress[cacheSize + blockSize - 1 : blockSize] :
	...
	//enable writing
	assign cacheWrite = fetchDone || (state == stateWRITE && cacheHit) || state == stateFLUSH || (state == stateCLEAN && writeclean);
	
	//If we want to read and we're either idle or reading and just had a hit or if we've just finished
	...
	// zahra - added dirty bit, changed write condition
	/*assign cacheData = 
	...
	assign cacheData = 
		(state == stateWRITE && cacheHit) ? 
			//{1'b1, cacheWriteData, avm_dataMaster_address[31 : cacheSize + blockSize], 1'b1} :
			{1'b1, cacheWriteData, zmemAddress[31 : cacheSize + blockSize], 1'b1} :
		state == stateFLUSH ? {(blockSizeBits + tagSizeBits + 2){1'b0}} :
		state == stateCLEAN ? {1'b0,cacheQ[blockSizeBits + tagSizeBits : 0]} :
			{1'b0, avm_dataMaster_readdata, fetchData, avm_dataMaster_address[31 : cacheSize + blockSize], 1'b1};
	...
	//modified for checkpoint and rollback
	reg [1:0] chstate;
	parameter nochreqs=0;
	parameter chings=1;
	parameter chcounts=2;
	reg [6:0] chcounter;
	parameter PIPELINEWAIT=3;
	...
	always @(posedge clk, negedge reset_n) begin
		if(!reset_n) begin
			...
			checkpointdone <= chdone;
			case (chstate)
				nochreqs:begin
					chdone <= 0;
					
					chcounter <= PIPELINEWAIT;
					if (checkpoint) begin
						chstate <= chcounts;
						checkpointing <= 1;
						chptdirtycounter<=0;
				...
				chcounts:begin
					...
					if (chcounter==0)begin
						chcounter<=PIPELINEWAIT;
						chstate<=chings;
						
						if ((state==stateIDLE && !memRead && !memWrite) || (state == stateREAD && cacheHit)
						) begin
							pipelineempty<=1;
						end
					end
				...
				chings:begin
					...
					if (writebackdirtydone==1)begin
						chdone <= 1;
						checkpointing <= 0;
						chstate <= nochreqs;
						pipelineempty<=0;
						chptdirtycounter<=chptdc;
					end
				end
			endcase
		end
	...
	always @(posedge clk, negedge reset_n) begin
		...
		//modified for checkpoint and rollback
		end else if (poweroff && ((state==stateIDLE && !memRead && !memWrite) || (state == stateREAD && cacheHit)) && !flushdone) begin
			state <= stateFLUSH;
			flushAddr <= 0;
		end else if (poweroff && flushdone) begin
			state <= stateIDLE;
		end else begin
			case(state)
				stateIDLE: begin
					...
					//modified for checkpoint and rollback
					zmemAddress <= memAddress;
					zmemWriteData <= memWriteData;	
					zmemWrite <= memWrite;
					zmemRead <= memRead;
					writebackdirtydone <= 0;
					chptdc<=0;
					...
					if(memRead) begin //If we want a read start a read
						...
						state <= stateFLUSH;
						flushAddr <= 0;
					end else if (pipelineempty & checkpointing & !writebackdirtydone) begin
						state <= stateWBDIRTYCACHE;
						flushAddr <= 0;
					end
				...
				stateREAD: begin
					...
					if(cacheHit) begin 
						...
						//modified for WB cache
						rmisscounter<=rmisscounter+1;
						if (dirtyBit)begin
							state <= stateWRITEBACK;
							writebackWord <= 0;
							avm_dataMaster_address <= {cacheTag, cacheAddress, {blockSize{1'b0}}};
							avm_dataMaster_byteenable <= 4'b1111;
							avm_dataMaster_writedata <= cacheQData[31 : 0];
				...
				stateFETCH: begin
					...
					if(avm_dataMaster_readdatavalid) begin
						...
						if(fetchWord == blockSize - 1) begin
							state <= stateREAD;
						end
					end
				end
				...
				stateWRITE: begin					
					...
					//modified for WB cache
					if (dirtyBit && !cacheHit) begin
						wmisscounter<=wmisscounter+1;
						state <= stateWRITEBACK;
						writebackWord <= 0;
						avm_dataMaster_address <= {cacheTag, zmemAddress[cacheSize + blockSize - 1 : blockSize], {blockSize{1'b0}}};
						avm_dataMaster_byteenable <= 4'b1111;
						avm_dataMaster_writedata <= cacheQData[31 : 0];
						avm_dataMaster_write <= 1;
					end
					else if (!dirtyBit && !cacheHit) begin
						wmisscounter<=wmisscounter+1;
						state <= stateFETCH_WRITE;
						avm_dataMaster_read <= 1;
						fetchWord <= 0;
						avm_dataMaster_address <= {zmemAddress[31:4], {blockSize{1'b0}}};
						avm_dataMaster_byteenable <= 4'b1111;
						address <= zmemAddress;
					end
				end
				...
				//modified for WB cache
				stateWRITEBACK: begin
					// Write miss happened. If block is dirty, writeback the current block. Fetch the new block from memory
					if(!avm_dataMaster_waitrequest && avm_dataMaster_write) begin
						case(writebackWord)
						2'b00: begin
							avm_dataMaster_writedata <= cacheQData[63 : 32];
							end
						2'b01: begin
							avm_dataMaster_writedata <= cacheQData[95 : 64];
							end
						2'b10: begin
							avm_dataMaster_writedata <= cacheQData[127 : 96];
							end
						endcase
						writebackWord <= writebackWord + 1;
						avm_dataMaster_address <= avm_dataMaster_address + 4;
						
						//If this is the last word go back to the previous state
						if(writebackWord == blockSize-1) begin
							avm_dataMaster_write <= 0;
				...
				stateWBDIRTY: begin
					if(!avm_dataMaster_waitrequest && avm_dataMaster_write) begin
						case(writebackWord)
						2'b00: begin
							avm_dataMaster_writedata <= zcacheQData[63 : 32];
							end
						2'b01: begin
							avm_dataMaster_writedata <= zcacheQData[95 : 64];
							end
						2'b10: begin
							avm_dataMaster_writedata <= zcacheQData[127 : 96];
							end
						endcase
						writebackWord <= writebackWord + 1;
						avm_dataMaster_address <= avm_dataMaster_address + 4;
						...
						if(writebackWord == blockSize-1) begin
							...
							if(flushAddr == {cacheSize{1'b1}}) begin
								//state <= stateIDLE;
								//writebackdirtydone <= 1;
								...
								state <= stateWBDIRTYCACHE;
								zflushAddr <= flushAddr;
							end
						end
					end
				end
				
				stateWBDIRTYCACHE: begin
					zcacheQ <= cacheQ;
					if (dirtyBit) begin
						chptdc<=chptdc+1;
						state <= stateWBDIRTY;
						writebackWord <= 0;
						avm_dataMaster_address <= {cacheTag, zflushAddr, {blockSize{1'b0}}};
						avm_dataMaster_byteenable <= 4'b1111;
						avm_dataMaster_writedata <= cacheQData[31 : 0];
						avm_dataMaster_write <= 1;
					end else begin
